...
export class ReceiptUtility {
        ...
        try {
            ...
            for (const item of inApps) {
                ...
                for (const field of fields) {
                    ...
                    if (fieldType !== TRANSACTION_IDENTIFIER_TYPE_ID &&
    ...
    extractTransactionIdFromTransactionReceipt(transactionReceipt) {
        ...
        const topLevelMatch = topLevel.match(/"purchase-info"\s+=\s+"([a-zA-Z0-9+/=]+)";/);
        if (!topLevelMatch)
            return null;
        const purchaseInfo = atob(topLevelMatch[1]);
        const purchaseInfoMatch = purchaseInfo.match(/"transaction-id"\s+=\s+"([a-zA-Z0-9+/=]+)";/);
        if (!purchaseInfoMatch)
            return null;
